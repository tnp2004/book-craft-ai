...
impl Book {
    ...
    fn read_book_response(json_str: &str) -> Result<BookContent> {
        let book_content: BookContent = serde_json::from_str(json_str)?;

        Ok(book_content)
    }

    pub async fn create_book(&self, resp: &str) -> Result<String> {
        let book_content = Self::read_book_response(resp)?;

        let images_path = self.generate_book_image(book_content.clone()).await?;
        ...
        File::create_html(&minified_html, &book_dir)?;

        Ok(book_dir)
    }

    async fn generate_book_image(&self, book_content: BookContent) -> Result<Vec<String>> {
        ...
        File::create_directory(&image_dir)?;

        let tasks: Vec<_> = book_content.story.into_iter()
        .map(|story| {
                ...
                tokio::spawn(async move {
                    ...
                    client.generate_image(&prompt, &dir).await.unwrap()
                })
        }).collect();

        
        let file_name_vec: Vec<String> = future::join_all(tasks).await.into_iter()
        .map(|res| {
